...
import { charityColorTailwindString, faq } from "~/utils/constants";

export const FAQAccordion = () => (
  <div className="mt-4 flex w-full flex-col items-stretch gap-1">
    <p
      className={`text-4xl font-black text-${charityColorTailwindString["buildOn"]}`}
    >
    ...
    {faq.map((faqSubject) => (
      <>
        <p
          className={`mt-2 text-2xl font-bold text-${charityColorTailwindString["buildOn"]}`}
        >
          {faqSubject.subject}
        </p>
        <Accordion.Root
...
const AccordionTrigger = React.forwardRef<{}, AccordionTriggerProps>(
  ({ children, className, ...props }, forwardedRef) => (
    <Accordion.Header className="flex w-full">
      <Accordion.Trigger
        className={classNames(
          `hover:bg-mauve2 group flex h-[45px] min-h-[45px] w-full flex-1 cursor-default items-center justify-between bg-white px-5 text-[15px] leading-none text-${charityColorTailwindString["buildOn"]} shadow-[0_1px_0] shadow-pompadour outline-none`,
          className
        )}
        {...props}
