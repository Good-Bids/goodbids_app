 ...
 * 
 */
const getCharities = async (charityStatus = "ACTIVE", windowStart = 0, windowLength = 25) => {
  ...
  try {
    ...
    if (charityStatus === "*") {
      ...
      result = await supabaseClient.from("charity")
        .select()
 ...
 * return types are inferred
 */
export const useCharitiesQuery = (charityStatus?: string | undefined, windowStart = 0, windowLength = 0) => {
